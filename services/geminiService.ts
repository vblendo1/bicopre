...
// Helper to clean JSON if the model adds Markdown fences
const cleanJson = (text: string) => {
  return text.replace(/```json/g, '').replace(/```/g, '').trim();
};

// Fallback data in case API fails or Key is missing
const MOCK_OPPORTUNITIES: GeneratedOpportunity[] = [
  {
    title: "Digitação de Cardápios para iFood",
    description: "Restaurantes locais precisam passar cardápio físico para digital. Trabalho manual simples e rápido.",
    price: "R$ 150,00",
    platform: "GetNinjas",
    postedAt: "Há 10 min",
    category: "Admin",
    url: "",
    actionStep: "Busque restaurantes novos no iFood e ofereça o serviço."
  },
  {
    title: "Teste de Usabilidade de App Bancário",
    description: "Empresa de UX paga para usuários comuns gravarem a tela tentando fazer um Pix.",
    price: "R$ 80,00/hora",
    platform: "Utest",
    postedAt: "Há 45 min",
    category: "Online",
    url: "",
    actionStep: "Cadastre-se na Utest e complete o perfil de hardware."
  },
  {
    title: "Cliente Oculto em Cafeteria",
    description: "Avalie o atendimento e a qualidade do café. Reembolso do consumo + cachê.",
    price: "R$ 60,00 + Lanche",
    platform: "Bare International",
    postedAt: "Há 2h",
    category: "Presencial",
    url: "",
    actionStep: "Procure projetos na sua cidade no site da Bare."
  }
];
...
export const generateOpportunities = async (count: number = 3, city?: string): Promise<GeneratedOpportunity[]> => {
  try {
    ...
    // Return mock data so the user sees the UI update even if API fails
